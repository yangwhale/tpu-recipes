...
# TPU: tpu7x-8x8x16 (1024 chips / 2048 devices / 256 hosts)
# Precision: bf16
#
# 基于 4x8x8 (256 chips) 配置扩展而来，主要变化：
#   - device-type: tpu7x-4x8x8 → tpu7x-8x8x16
#   - 预期 GBS: 16384 (= 2048 devices × 8 per_device_batch_size)
#   - 并行策略保持不变: ici_fsdp_transpose=2, use_2d_fsdp_sharding=True
#     ici_fsdp 自动计算: 2048 / (1 × 2) = 1024
#   - XLA_FLAGS 与 4x8x8 相同（无新增 flags）
#
# 资源需求：
#   - 1024 chips = 256 hosts × 4 chips/host
#   - Placement policy: tpu7x-2048-8x8x16-placement-policy (已存在)
#   - 需要 Kueue ResourceFlavor: 1xtpu7x-2048
#   - 需要 xpk configmap 条目: tpu7x-2048: "256"
# =============================================================================
...
export WORKLOAD_NAME="$(printf "%.26s" "${USER//_/-}-ds3-bf16-8x8x16")-$(date +%Y%m%d-%H%M)"

echo "=========================================="
echo "Submitting DeepSeek3-671B BF16 Training (8x8x16)"
echo "=========================================="
echo "Workload Name: ${WORKLOAD_NAME}"
echo "Docker Image:  ${WORKLOAD_IMAGE}"
echo "TPU Type:      tpu7x-8x8x16 (1024 chips / 2048 devices / 256 hosts)"
echo "Precision:     bf16"
...
# MaxText 参数 (基于 4x8x8 bf16，保持相同并行策略)
# 关键并行参数：
#   ici_fsdp_transpose_parallelism=2 (与 4x8x8 相同)
#   ici_fsdp_parallelism=-1 (auto = 2048 / 1 / 2 = 1024)
#   use_2d_fsdp_sharding=True
#   fsdp_shard_on_exp=False
# 预期 GBS = 2048 devices × 8.0 = 16384
MAXTEXT_ARGS="\
